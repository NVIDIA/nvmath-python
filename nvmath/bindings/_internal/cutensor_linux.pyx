...
cdef int _check_or_init_cutensor() except -1 nogil:
    ...
    with gil, __symbol_lock:
        ...
        __cutensorPlanPreferenceGetAttribute = dlsym(RTLD_DEFAULT, 'cutensorPlanPreferenceGetAttribute')
        if __cutensorPlanPreferenceGetAttribute == NULL:
            if handle == NULL:
                handle = load_library()
            __cutensorPlanPreferenceGetAttribute = dlsym(handle, 'cutensorPlanPreferenceGetAttribute')
