...
def validate(size, precision, fft_type, execution, direction, ffts_per_block, elements_per_thread, real_fft_options, code_type):
    ...
    if ffts_per_block is None:
        ...
        if ffts_per_block <= 0:
            raise ValueError(f"ffts_per_block must be None, 'suggested' or a positive integer ; got ffts_per_block = {ffts_per_block}")
    if elements_per_thread is None:
        ...
        if elements_per_thread <= 0:
            raise ValueError(f"elements_per_thread must be None, 'suggested' or a positive integer ; got elements_per_thread = {elements_per_thread}")
...
def generate_FFT(size, precision, fft_type, direction, code_type, execution, ffts_per_block, elements_per_thread, real_fft_options):
    ...
    name = json_hash(size=size, precision=NP_TYPES_TO_CPP_TYPES[precision], fft_type=fft_type, direction=direction, sm=sm, \
                        ffts_per_block=ffts_per_block, elements_per_thread=elements_per_thread, \
...
def generate_block(size, precision, fft_type, direction, code_type, ffts_per_block, elements_per_thread, real_fft_options):
    ...
    FFT, name = generate_FFT(size=size, precision=precision, fft_type=fft_type, direction=direction, code_type=code_type, execution='Block', \
                             ffts_per_block=ffts_per_block, elements_per_thread=elements_per_thread, \
                             real_fft_options=real_fft_options)
...
def generate_thread(size, precision, fft_type, direction, code_type, real_fft_options):
    ...
    FFT, name = generate_FFT(size=size, precision=precision, fft_type=fft_type, direction=direction, code_type=code_type, execution='Thread', \
                             ffts_per_block=None, elements_per_thread=None, real_fft_options=real_fft_options)
    thread_api_name = f"libmathdx_function_fft_thread_{name}"
