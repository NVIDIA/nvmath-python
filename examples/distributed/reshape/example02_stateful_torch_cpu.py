...
memory for distributed reshaping.

The input as well as the result from the Reshape operations are Torch tensors on CPU.
...
with nvmath.distributed.reshape.Reshape(a, input_box, output_box) as r:
    ...
    if rank == 0:
        ...
        print(f"Reshape output type = {type(b)}")
