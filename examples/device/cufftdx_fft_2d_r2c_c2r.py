...
def main():
    ...
    FFT_x_c2c_f = FFT_base(fft_type='c2c', direction='inverse', size=fft_size_x, elements_per_thread=ept_x, ffts_per_block=fpb_x)
    # C2Ci along X (fft_size_y//2+1 batches, logical FFT size is fft_size_x)
    FFT_x_c2c_i = FFT_base(fft_type='c2c', direction='forward', size=fft_size_x, elements_per_thread=ept_x, ffts_per_block=fpb_x)
