...
from pynvjitlink import patch; patch.patch_numba_linker()
...
def main():
    ...
    FFT = fft(fft_type='c2c', size=m * n, precision=np.float32, direction='forward',
              elements_per_thread=2, ffts_per_block=1, execution='Block', compiler='numba')

    MM = matmul(size=(m, n, k), precision=np.float32, data_type='complex',
                transpose_mode=('non_transposed', 'non_transposed'),
                execution='Block', block_dim=FFT.block_dim, compiler='numba')
