...
def main():
    ...
    FFT = fft(fft_type='c2c', size=k, precision=np.float16, direction='forward',
              elements_per_thread=2, ffts_per_block=batch_size, execution='Block', compiler='numba')

    assert FFT.block_dim.y == 1
    assert FFT.ffts_per_block == FFT.implicit_type_batching

    MM = matmul(size=(m, n, k), precision=np.float16, data_type='complex',
                transpose_mode=('non_transposed', 'non_transposed'),
                execution='Block', block_size=FFT.block_dim.x, compiler='numba')
