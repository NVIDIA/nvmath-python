...
are UST backed by torch tensors (on the CPU or GPU). The user can choose an
appropriate index type for the problem - for example, a sparse matrix with
very large extents needs an `int64` index type.
...
index_type, dtype = torch.int64, torch.float32
