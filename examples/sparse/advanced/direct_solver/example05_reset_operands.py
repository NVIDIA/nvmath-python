...
The LHS is a CuPy CSR matrix while the RHS and solution are CuPy ndarrays on the GPU.
"""
...
with nvmath.sparse.advanced.DirectSolver(a, b) as solver:
    ...
    # 'A' can also be updated out-of-place and operands reset. In this case
    # we need to re-plan since the crow_indices, col_indices pointer values
    # have changed, even if the data remains the same. Therefore, in-place
    # updates are recommended for A to avoid the need for re-planning.
    a = a / 10
