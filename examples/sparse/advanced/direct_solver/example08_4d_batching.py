...
since it's currently the only package that supports CSR sparse tensors (>= 3D). There are
multiple (two) batch dimensions in this example. It uses operands on the GPU, and CPU
operands can be used instead by setting `device_id = "cpu"` below.
"""
...
b = torch.ones(batch, batch, 3, n, device=device_id).permute(0, 1, 3, 2)
