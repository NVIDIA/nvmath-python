...
an existing sparse SciPy COO array.
...
# Create a SciPy COO matrix on the CPU.
indices = np.array([[0, 1], [0, 1]], dtype=np.int32)
values = np.array([2.0, 4.0], dtype=np.float64)
shape = 2, 2
a = sp.coo_array((values, (indices[0], indices[1])), shape=shape)
...
a.sum_duplicates()
print(f"The SciPy COO matrix is:\n {a}.")

# Create an UST from the SciPy COO matrix. The `from_package` method is zero-copy (it
...
a.data[0] = 3.14
print(f"\nThe modified SciPy COO matrix is:\n {a}.")
