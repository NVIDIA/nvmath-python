...
an existing PyTorch strided tensor.  The layout is neither `C` (right) nor `F`
(left). Currently, the UST requires that the strided tensor is dense (no holes)
...
shape = 2, 4, 3
a = torch.rand(*shape, dtype=torch.float64, device=device_id).permute(0, 2, 1)
