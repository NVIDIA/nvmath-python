...
def get_framework_from_array(array: np.ndarray | CP_NDARRAY | TORCH_TENSOR):
    if isinstance(array, np.ndarray):
        return Framework.numpy
    elif CP_NDARRAY is not None and isinstance(array, CP_NDARRAY):
...
def get_dtype_from_array(array: np.ndarray | CP_NDARRAY | TORCH_TENSOR) -> DType:
    framework = get_framework_from_array(array)
    return framework_dtype_rev[framework][array.dtype]


def get_array_backend(array: np.ndarray | CP_NDARRAY | TORCH_TENSOR):
    if isinstance(array, np.ndarray):
        return MemBackend.cpu
    elif CP_NDARRAY is not None and isinstance(array, CP_NDARRAY):
        return MemBackend.cuda
    elif TORCH_TENSOR is not None and isinstance(array, TORCH_TENSOR):
        return MemBackend.cuda if array.is_cuda else MemBackend.cpu
    else:
        raise ValueError(f"Unknown array type {array}")
...
def is_array(array):
    types = tuple(t for t in (np.ndarray, CP_NDARRAY, TORCH_TENSOR) if t is not None)
    return isinstance(array, types)
