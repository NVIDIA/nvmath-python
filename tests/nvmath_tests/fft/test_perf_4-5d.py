...
def run_test():
    ...
    cols = ['axes', 'size', 'batch', 'dataset_size [MiB]', 'cupy [ms]', 'nvmath-python [ms]', 'nvmath-python-caching [ms]', 'cupy [GFlop/s]', 'nvmath-python-noncaching [GFlop/s]', 'nvmath-python [GFlop/s]', 'speedup nvmath-python over cupy']
    data = []
    ...
    for num_axes in range(start, rank + 1):
        ...
        for axes in list(p for c in itertools.combinations(range(rank), num_axes) for p in itertools.permutations(c)):
            ...
            data.append({
                ...
                'nvmath-python-noncaching [GFlop/s]': fft_perf_GFlops(fft_size, batch_size, time_nvmath_noncaching['time_ms']),
