...
COMPLEX_TYPE_MAP = {
    np.float16: float16x4_type,
    np.float32: float32x2_type,
    np.float64: float64x2_type
}
...
def test_block(fft_type, size, precision, direction, api_kind, ept, real_fft_options):
    ...
    FFT = fft(fft_type=fft_type, ffts_per_block=ffts_per_block, elements_per_thread=ept,
              size=size, precision=precision, direction=direction,
              real_fft_options=real_fft_options, execution='Block', compiler='numba')
        ...
        for i in range(elements_per_thread):
            ...
            if idx < OUTPUT_SIZE:
                if IS_FP16:
                    ...
                    if IS_SMEM:
                        if OUTPUT_FOLDED:
                            output[global_fft_id, 2 * idx + 0], output[global_fft_id, 2 * idx + 1] = shared_mem_output[smem_idx].x, shared_mem_output[smem_idx].y
                        else:
                            output[global_fft_id, idx] = shared_mem_output[smem_idx]
                    else:
                        if OUTPUT_FOLDED:
                            output[global_fft_id, 2 * idx + 0], output[global_fft_id, 2 * idx + 1] = thread_output[i].x, thread_output[i].y
                        else:
                            output[global_fft_id, idx] = thread_output[i]

    input_h, output_ref, input_d, output_d = make_numba_input_outputs(fft_type, precision, direction, num_batches, size, real_fft_options)
...
@pytest.mark.parametrize("fft_type,size,precision,direction,real_fft_options", [
...
def test_thread(fft_type, size, precision, direction, real_fft_options):
    ...
    FFT = fft(fft_type=fft_type, size=size, precision=precision, direction=direction,
              real_fft_options=real_fft_options, execution='Thread', compiler='numba')
    show_FFT_traits(FFT)
    ...
    input_h, output_ref, input_d, output_d = make_numba_input_outputs(fft_type, precision, direction, implicit_type_batching, size, real_fft_options)
...
def test_valid():
    ...
    base_FFT = FFTOptions(fft_type='c2c', size=2, precision=np.float32, direction='forward', execution='Block', code_type=current_device_lto())
