...
def run_conv_perf(test_cases):
    ...
    for size, precision in test_cases:
        ...
        # Numba
        #
        numba_test = FFTConvNumba(
            size=size,
            precision=precision,
            fft_type="c2c",
            ffts_per_block=ffts_per_block,
            elements_per_thread=elements_per_thread,
            use_vectorized_load_store=False,
        )
        ...
        # Numba vectorized
        #
        numba_vectorized_test = FFTConvNumba(
            size=size,
            precision=precision,
            fft_type="c2c",
            ffts_per_block=ffts_per_block,
            elements_per_thread=elements_per_thread,
            use_vectorized_load_store=True,
        )
...
if __name__ == "__main__":
    ...
    size = args.size
    PREC_MAP = {"float32": np.float32, "float64": np.float64}
