...
@pytest.mark.parametrize("fft_type, complex_layout, real_mode", [
    ('r2c', 'natural', 'folded'),
    ('r2c', 'packed',  'folded'),
    ('r2c', 'full',    'folded'),
    ('r2c', 'natural', 'normal'),
...
def test_knobs_r2c_c2r(fft_type, complex_layout, real_mode):
    ...
    FO = FFTOptions(fft_type=fft_type, size=512, precision=np.float32, code_type=SM80, execution='Block',
...
def test_knobs_1():
    ...
    FO = FFTOptions(fft_type='c2c', size=4, precision=np.float32, direction='forward', code_type=SM80, execution='Block', ffts_per_block='suggested', elements_per_thread='suggested')
    ...
    FFT = fft(fft_type='c2c', size=4, precision=np.float32, direction='forward', code_type=SM80, execution='Block', ffts_per_block=ffts_per_block)
    assert isinstance(FFT, FFTCompiled)
    assert FFT.ffts_per_block == ffts_per_block

def test_knobs_2():

    FO = FFTOptions(fft_type='c2c', size=4, precision=np.float32, direction='forward', code_type=SM80, execution='Block', ffts_per_block='suggested')
    assert FO.ffts_per_block is not None
...
def test_knobs_3():
    ...
    FO = FFTOptions(fft_type='c2c', size=4, precision=np.float32, direction='forward', code_type=SM80, execution='Block', elements_per_thread='suggested')
...
def test_partial_fft():
    FO = FFTOptions(fft_type='c2c', size=32, precision=np.float32, direction='forward', \
                     code_type=SM80, execution='Block', ffts_per_block='suggested')
    suggested_ffts_per_block = FO.ffts_per_block

    FFT = fft(fft_type='c2c', size=32, precision=np.float32, direction='forward', \
              code_type=SM80, execution='Block', ffts_per_block=suggested_ffts_per_block)
    assert isinstance(FFT, FFTCompiled)
    assert FFT.ffts_per_block == suggested_ffts_per_block

def test_valid_knobs():
    FO = FFTOptions(fft_type='c2c', size=32, precision=np.float32, direction='forward', \
                    code_type=SM80, execution='Block')
    valids = FO.valid('elements_per_thread', 'ffts_per_block')
    count = 0
    for (ept, bpb) in valids:
        count += 1
        FFT = fft(fft_type='c2c', size=32, precision=np.float32, direction='forward', \
                  code_type=SM80, execution='Block', elements_per_thread=ept, ffts_per_block=bpb)
        assert isinstance(FFT, FFTCompiled)
        assert FFT.elements_per_thread == ept
...
@pytest.mark.parametrize("precision,value_type", [
    (np.float16, np.dtype([('x', np.float16), ('y', np.float16), ('z', np.float16), ('w', np.float16)], align=True)), # ~ complex<__half2>
...
def test_value_type(precision, value_type):
    for fft_type in ['c2r', 'r2c', 'c2c']:
        FFT = fft(fft_type=fft_type, size=256, precision=precision, direction='forward' if fft_type == 'c2c' else None, code_type=SM90, execution='Block')
