...
@pytest.mark.parametrize("shape,block_size,block_dim,data_type,trans,precision,np_type,numba_type,explicit_ld", [ \

# Various data_types and T/N/C
    ((16, 4, 2),      32,   None, 'complex', ('conj_transposed', 'conj_transposed'), np.float32, np.complex64,                                     float32x2_type, True), \
    ((4, 8, 2),       32,   None, 'complex', ('conj_transposed', 'conj_transposed'), np.float32, np.complex64,                                     float32x2_type, False), \
    ((16, 32, 2),     32,   None, 'complex', ('non_transposed', 'non_transposed'),   np.float16, np.dtype([('x', np.float16), ('y', np.float16)]), float16x2_type, False), \
    ((4, 32, 8),      32,   None, 'complex', ('non_transposed', 'non_transposed'),   np.float16, np.dtype([('x', np.float16), ('y', np.float16)]), float16x2_type, False), \
    ((4, 16, 2),      128,  None, 'complex', ('non_transposed', 'non_transposed'),   np.float32, np.complex64,                                     float32x2_type, False), \
    ((2, 2, 32),      256,  None, 'complex', ('non_transposed', 'non_transposed'),   np.float64, np.complex128,                                    float64x2_type, True), \
    ((16, 16, 16),    32,   None, 'real',    ('non_transposed', 'non_transposed'),   np.float32, np.float32,                                       np.float32,     False), \
    ((8, 8, 8),       32,   None, 'real',    ('transposed', 'non_transposed'),       np.float64, np.float64,                                       np.float64,     False), \
    ((16, 16, 16),    32,   None, 'real',    ('non_transposed', 'non_transposed'),   np.float16, np.float16,                                       np.float16,     False), \
...
# 2D+ block_dim
    ((15, 17,  19),     None, (32,1,1),   'real',    ('transposed', 'non_transposed'),       np.float32, np.float32,                                       np.float32,     False), \
    ((1,  59,   1),     None, (33,1,1),   'real',    ('transposed', 'non_transposed'),       np.float32, np.float32,                                       np.float32,     False), \
    ((59,  1,   1),     None, (1023,1,1), 'real',    ('transposed', 'non_transposed'),       np.float32, np.float32,                                       np.float32,     False), \
    ((1,   1,  17),     None, (4,16,1),    'real',    ('transposed', 'non_transposed'),       np.float32, np.float32,                                       np.float32,     False), \
    ((1,   1,  17),     None, (4,4,4),    'real',    ('transposed', 'non_transposed'),       np.float32, np.float32,                                       np.float32,     False), \
    ((3,   3,   3),     None, (5,7,3),    'real',    ('transposed', 'non_transposed'),       np.float32, np.float32,                                       np.float32,     False), \
])
def test_matmul(shape, block_size, block_dim, data_type, trans, precision, np_type, numba_type, explicit_ld):
    ...
    SM = set_device()
    MM = time_this('matmul codegen', matmul, size=(m, n, k), data_type=data_type, precision=precision, \
                   transpose_mode=TransposeMode(a_trans, b_trans), block_size=block_size, \
                   block_dim=block_dim, execution='Block', compiler='numba')
    show_MM_traits(MM)
...
def test_valid():
    ...
    base_MM = BlasOptions(size=(8, 4, 16), data_type='real', precision=np.float32, \
                    transpose_mode=TransposeMode('transposed', 'non_transposed'), \
                    execution='Block', code_type=current_device_lto())
