...
import numpy as np
import time
from nvmath.device import Dim3, CodeType, ComputeCapability, matmul, BlasOptions, TransposeMode, LeadingDimension, BlasOptions
...
def test_third_party_symbols():
    ...
    MM = matmul(size=(24, 8, 48), data_type='real', precision=np.float64, \
                transpose_mode=TransposeMode('non_transposed', 'transposed'), \
                code_type=SM70, execution='Block')
...
def test_third_party_code():
    ...
    MM = matmul(size=(16, 8, 16), data_type='real', precision=np.float32, \
                transpose_mode=TransposeMode('non_transposed', 'transposed'), \
                code_type=SM70, execution='Block')
...
@pytest.mark.parametrize("ta, tb",
                         list(itertools.product(['non_transposed', 'transposed', 'conj_transposed'], repeat=2)))
def test_transpose_mode(ta, tb):

    MM1 = matmul(size=(2, 2, 2), data_type='complex', precision=np.float32, \
                transpose_mode=(ta, tb), \
                code_type=SM70, execution='Block')

    MM2 = matmul(size=(2, 2, 2), data_type='complex', precision=np.float32, \
...
def test_suggested_block_dim():
    ...
    BO = BlasOptions(size=(16, 8, 16), data_type='real', precision=np.float32, \
                     transpose_mode=TransposeMode('non_transposed', 'transposed'), \
                     code_type=SM70, execution='Block', block_dim='suggested') # leading_dimension = None implicit
...
def test_suggested_leading_dimension():
    ...
    BO = BlasOptions(size=(16, 8, 16), data_type='real', precision=np.float32, \
                     transpose_mode=TransposeMode('non_transposed', 'transposed'), \
                     code_type=SM89, block_size=64, execution='Block', leading_dimension='suggested')
...
def test_cached():
    ...
    make_mm = functools.partial(matmul, \
                                size=(32, 16, 32), data_type='real', precision=np.float32, \
                                transpose_mode=TransposeMode('transposed', 'transposed'), \
                                block_dim=Dim3(2, 4, 8), code_type=SM70, execution='Block', compiler=None)
...
@pytest.mark.parametrize("data_type,precision,value_type", [
